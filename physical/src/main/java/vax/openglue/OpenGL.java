...
/**
 Basic OpenGL method interface, intended be be roughly equal to OpenGL 3.0/OpenGL ES 2.0 (with some minor exceptions).
 <p>
 NOTE: don't place data[] calls here - JNI calls for arrays require copying one way or another (either to direct buffer, or in JNI invoc);
 as such, it's a bad idea to place them without explicit warnings - so it's IMO better to not place them at all
...
public interface OpenGL {
    ...
    /*
     glBufferData variants - required for LWJGL compatibility
     */
    void glBufferData ( int bufferTarget, ByteBuffer data, int usageEnum );

    void glBufferData ( int bufferTarget, long size, ByteBuffer data, int usageEnum );

    void glBufferData ( int bufferTarget, long size, ShortBuffer data, int usageEnum );

    void glBufferData ( int bufferTarget, long size, IntBuffer data, int usageEnum );
